...
import com.lmbibbo.email.EmailService;
...
public class RegistrationService {
    ...
    public String register(RegistrationRequest request) {
        ...
        String link = "http://localhost:8080/api/auth/registration/confirm?token=" + token;
//        emailService.send(request.getEmail(), buildEmail(request.getName(), link));
    ...
    public String confirmToken(String token) {
        ...
        ApplicationUser applicationUser =
                applicationUserRepository.findByConfirmationToken_Token(token).
                        orElseThrow(() ->
                                new IllegalStateException(String.format("User not found with Token: %s", token))
                        );
        ConfirmationToken confirmationToken = applicationUser.getConfirmationToken();
    ...
    private String buildEmail(String name, String link) {
        return "<div style=\"font-family:Helvetica,Arial,sans-serif;font-size:16px;margin:0;color:#0b0c0c\">\n" +
                ...
                "               <p style=\"Margin:0 0 20px 0;font-size:19px;line-height:25px;color:#0b0c0c\"> Thank you for registering. Please click on the below link to activate your account: </p><blockquote style=\"Margin:0 0 20px 0;border-left:10px solid #b1b4b6;padding:15px 0 0.1px 15px;font-size:19px;line-height:25px\">" +
